...
pub enum BlockKind {
    /// Special block which is not supposed to be popped. Used to get return values in tests.
    Top,
    Block,
    Loop,
    Fun,
...
impl Stack {
    ...
    pub fn pop_value_opt(&mut self) -> Result<Option<Value>> {
    ...
    pub fn push_f32(&mut self, f: f32) -> Result<()> {
    ...
    pub fn push_loop(&mut self, cont: u32, n_args: u32, n_rets: u32) {
        self.0.push(Block {
            values: vec![],
            cont,
            n_args,
    ...
    pub fn push_fun_block(&mut self, cont: u32, n_args: u32, n_rets: u32) {
        self.0.push(Block {
            values: vec![],
            cont,
            n_args,
    ...
    pub fn pop_fun_block(&mut self) -> Result<Block> {
        loop {
    ...
    pub fn return_arity(&mut self, depth: u32, end_or_break: EndOrBreak) -> Result<u32> {
